...
- `/checks`：逐项记录检查结论、说明和证据；支持勾选多个待检项批量复核，每项各自选择通过或未通过、共用一份证据和说明，服务端在单事务内加锁校验并整批写入，任一项已被复核或证据无效时整批失败、不留部分结论；检查复核使用事务锁且结论不可改写，未处理或失败检查会阻断完全放行。
...
| PATCH | `/checks/:id/review` | 提交单项结论和证据 | 检查角色 |
| POST | `/checks/batch-review` | 批量复核：多项各自给结论、共用一份证据和说明，单事务整批成败 | 检查角色 |
